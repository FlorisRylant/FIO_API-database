...
from market_fetcher import get_ask
...
def calculate_upkeep(workers, luxury={}, market_identifier='NC1', logging=False):
    ...
    for item in needed_items:
        cost += needed_items[item]*get_ask(item, market_identifier, logging)
    return round(cost)
...
def upkeep_rapport(workers, luxury={}, market_identifier='NC1', logging=False):
    ...
    supply_prices = {}
    for item in items_needed: supply_prices[item] = get_ask(item, market_identifier, logging)
    if logging: print()
    total = 0
    for work_type in workers:
